...
(function() {
    ...
    if (location.href.includes("spacedrevision.com")) {
        ...
        function mcqFieldIndexFromLabelText(t) {
            const num = t.match(/[1-4]/);
            if (num) return Number(num[0]) - 1;
            const letter = t.match(/(?:option|explanation)\s*(?:for\s+option\s*)?([a-d])\b/);
            return letter ? letter[1].charCodeAt(0) - 97 : -1;
        }

        function mcqFieldFromLabelText(labelText) {
            const t = (labelText || '').replace(/\s+/g, ' ').toLowerCase();
            const index = mcqFieldIndexFromLabelText(t);
            // "Explanation" wins over "Option" so labels like "Explanation for
            // option 2" or "Option 2 explanation" resolve to the explanation.
            if (t.includes('explanation')) {
                return index >= 0 ? { kind: 'explanation', index } : null;
            }
            if (t.includes('option')) {
                return index >= 0 ? { kind: 'option', index } : null;
            }
            if (t.includes('question')) return { kind: 'question', index: -1 };
            return null;
        }

        // A label sits immediately before the field it names, so walk backwards
        // from the editor through preceding labels and take the first one we
        // recognise. Sniffing an ancestor's text instead matches both "Option N"
        // and "Explanation N" from the same option row.
        function mcqFieldForEditor(editorEl, labels) {
            for (let i = labels.length - 1; i >= 0; i--) {
                const label = labels[i];
                const pos = label.compareDocumentPosition(editorEl);
                if (!(pos & Node.DOCUMENT_POSITION_FOLLOWING)) continue;
                const labelText = (label.textContent || '').replace(/\s+/g, ' ').trim();
                const field = mcqFieldFromLabelText(labelText);
                if (field) return { kind: field.kind, index: field.index, labelText };
            }
            return null;
        }

        function contentForMcqField(field, data) {
            if (!field) return '';
            if (field.kind === 'question') return data.question || '';
            if (field.kind === 'option') return (data.options && data.options[field.index]) || '';
            if (field.kind === 'explanation') return (data.explanations && data.explanations[field.index]) || '';
            return '';
        }

        function mcqFieldsForEditors(editors) {
            const labels = Array.from(document.querySelectorAll('label'));
            const fields = editors.map(editor => mcqFieldForEditor(editor, labels));
            const seen = new Set();
            for (const field of fields) {
                if (!field) continue;
                const key = field.kind + ':' + field.index;
                if (seen.has(key)) return null;
                seen.add(key);
            }
            // Without option fields the labels were misread, so let the caller
            // fall back to the fixed editor order rather than paste half a form.
            if (!fields.some(f => f && f.kind === 'option')) return null;
            return fields;
        }

        function logMcqPaste(editors, fields, data) {
            const clip = s => (s || '').toString().replace(/\s+/g, ' ').slice(0, 60);
            console.log('[MCQ Bridge] payload', {
                question: clip(data.question),
                options: (data.options || []).map(clip),
                explanations: (data.explanations || []).map(clip),
                correctOption: data.correctOption || '',
            });
            console.log('[MCQ Bridge] editor → field mapping', editors.map((editor, i) => {
                const field = fields && fields[i];
                return {
                    editor: i,
                    label: field ? field.labelText : '(unmatched)',
                    field: field ? field.kind + ' ' + (field.kind === 'question' ? '' : field.index + 1) : 'by DOM order',
                };
            }));
        }
        ...
        async function pasteMcqEditors(data) {
            ...
            const fields = mcqFieldsForEditors(editors);
            logMcqPaste(editors, fields, data);

            // Both MCQ form layouts are fixed: Q, O1, E1, O2, E2, O3, E3, O4, E4.
            ...
            if (!fields && editors.length < 9) {
                console.warn('[MCQ Bridge] Could not match editors to Question/Option/Explanation labels; found ' + editors.length + ' editors.');
                return;
            }

            for (let i = 0; i < editors.length; i++) {
                const content = fields
                    ? contentForMcqField(fields[i], data)
                    : (orderedContents[i] || '');
                if (content) {
                    pasteIntoEditor(editors[i], mdToHtml(content));
                    await delay(250);
                }
            }
        }
